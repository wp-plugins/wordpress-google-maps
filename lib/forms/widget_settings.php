...
<p class="agm_widget_query_options">
	...
	<br />
	<input id="<?php echo $this->get_field_id('query_all'); ?>" name="<?php echo $this->get_field_name('query'); ?>" type="radio" <?php echo (('all_posts' == $query) ? 'checked="checked"' : ''); ?> value="all_posts" />
		<label for="<?php echo $this->get_field_id('query_all'); ?>"><?php _e('All posts', 'agm_google_maps'); ?></label>
	<br />
...
<p>
	...
	<br />
	<input disabled="disabled" id="<?php echo $this->get_field_id('show_map'); ?>" name="<?php echo $this->get_field_name('show_map'); ?>" <?php echo (($show_map) ? 'checked="checked"' : ''); ?> type="checkbox" value="1" />
		<label for="<?php echo $this->get_field_id('show_map'); ?>"><?php _e('Show map', 'agm_google_maps'); ?></label>
	<br />
	<input disabled="disabled" id="<?php echo $this->get_field_id('show_markers'); ?>" name="<?php echo $this->get_field_name('show_markers'); ?>" <?php echo (($show_markers) ? 'checked="checked"' : ''); ?> type="checkbox" value="1" />
		<label for="<?php echo $this->get_field_id('show_markers'); ?>"><?php _e('Show marker list', 'agm_google_maps'); ?></label>
	<br />
	<input disabled="disabled" id="<?php echo $this->get_field_id('show_images'); ?>" name="<?php echo $this->get_field_name('show_images'); ?>" <?php echo (($show_images) ? 'checked="checked"' : ''); ?> type="checkbox" value="1" />
...
function toggleTargets (e) {
	...
	if ($customSwitch.is(':checked')) $customTarget.attr('disabled', false);
	else disable($customTarget);
}
