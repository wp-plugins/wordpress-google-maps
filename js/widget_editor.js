...
function createNewMap () {
	...
	window.tb_show(l10nEditor.add_map, '#TB_inline?width=640&height=' + height + '&inlineId=map_container');
	$.post(window.ajaxurl, {"action": 'agm_new_map'}, function (data) {
		...
		_mapHandler = new window.AgmMapHandler("#map_preview_container", data, false);
	});
	return false;
}
...
$('#map_preview_container').bind('agm_map_saved', function (e, mapId) {
	...
	$.post(window.ajaxurl, {"action": "agm_list_maps"}, function (data) {
