...
function requestMapList () {
	...
	$.post(window.ajaxurl, data, loadMaps);
}
...
function deleteMap () {
	...
	$.post(window.ajaxurl, {"action": "agm_delete_map", "id": mapId}, function (data) {
		requestMapList();
	});
}
...
function insertAtCursor(fld, text) {
    ...
    // Rest
    else if (fld.selectionStart || parseInt(fld.selectionStart) === 0) {
        var startPos = fld.selectionStart;
        var endPos = fld.selectionEnd;
        fld.value = fld.value.substring(0, startPos) + text + fld.value.substring(endPos, fld.value.length);
    } else {
    	fld.value += text;
    }
}
...
function updateMapPreview () {
	...
	$.post(window.ajaxurl, {"action": 'agm_load_map', "id": id}, function (data) {
		if (_mapHandler) { _mapHandler.destroy(); }
		_mapHandler = new window.AgmMapHandler("#map_preview_container", data);
	});
	$('#maps_existing').hide();
	$('#maps_new').hide();
}
 ...
 */
function createMap () {
	$.post(window.ajaxurl, {"action": 'agm_new_map'}, function (data) {
...
function advancedModeOn () {
	...
	$('#maps_merge_locations').click(function () {
		...
		$.post(window.ajaxurl, {"action": "agm_merge_maps", "ids": mapIds}, function (data) {
	...
	$('#maps_batch_delete').click(function () {
		...
		$.post(window.ajaxurl, {"action": "agm_batch_delete", "ids": mapIds}, function (data) {
			$('#maps_advanced_switch').click(); // Turn off advanced mode
			requestMapList();
		});
	});
...
mbuttons_container.append('' +
	'<a onclick="return openMapEditor();" title="' + l10nEditor.add_map + '" class="thickbox" id="add_map" href="#TB_inline?width=640&height=594&inlineId=map_container">' +
		'<img onclick="return false;" alt="' + l10nEditor.add_map + '" src="' + window._agm_root_url + '/img/system/globe-button.gif">' +
	'</a>'
);
...
$('body').append(
	...
	(window._agm_is_multisite ? '' : '<p class="agm_less_important">For more detailed instructions on how to use refer to <a target="_blank" href="http://premium.wpmudev.org/project/wordpress-google-maps-plugin/installation/">Google Maps Installation and Use instructions</a>.</p>') +
		...
		'<div class="agm_container" id="maps_existing">' +
			...
			'<div id="maps_existing_result"><img src="' + window._agm_root_url + '/img/system/loading.gif" />' + l10nEditor.loading + '</div>' +
